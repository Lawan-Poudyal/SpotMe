...
export default function FindMeTab({
  ...
  // ── "Photos of me" (facial-recognition matches) ────────
  const [lightboxIndex, setLightboxIndex] = useState(-1);
  // Lets the user step back into the upload flow even after we already
  // have a reference photo and matches for them.
  const [isReplacingReference, setIsReplacingReference] = useState(false);

  // A reference photo exists either because the event already had one on
  // record, or because the one just staged in this session finished
  // uploading.
  const hasReference = !!existingReferencePhoto || referenceFile?.status === 'done';
  const showUploadFlow = !hasReference || isReplacingReference;

  const myPhotosQueryKey = ['myPhotos', eventId, userId];

  const { data: myPhotosData, isLoading: isMyPhotosLoading } = useQuery({
    queryKey: myPhotosQueryKey,
    queryFn: () => photo.getMyPhoto(eventId as string, String(userId)),
  ...
  // for the relevant event — invalidate the matches query so it refetches
  // on its own instead of us guessing when processing is actually done.
  useEffect(() => {
    if (!socket) return;

    const handleReferenceEmbeddingNews = (payload: ReferenceEmbeddingNews) => {
      if (!payload?.success) return;
      if (String(payload.eventId) !== String(eventId)) return;
      queryClient.invalidateQueries({ queryKey: myPhotosQueryKey });
    };

    socket.on('reference_embedding_news', handleReferenceEmbeddingNews);
    return () => {
      socket.off('reference_embedding_news', handleReferenceEmbeddingNews);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [socket, eventId, userId]);

  // After a fresh upload finishes, drop back out of "replacing" mode so
  // the matches view takes over. The matches themselves refresh once the
  // 'reference_embedding_news' event above fires, not here.
  const handleUploadClick = async () => {
    await handleReferenceUpload();
    ...
    <div>
      ...
      {!showUploadFlow ? (
        <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-white/50 text-xs font-medium uppercase tracking-wide">
                {isMyPhotosLoading
                  ? 'Looking for you…'
                  : myPhotos.length > 0
                    ? `${myPhotos.length} photo${myPhotos.length === 1 ? '' : 's'} of you`
                    : 'No matches yet'}
              </p>
            </div>
            <button
              onClick={() => setIsReplacingReference(true)}
              className="text-[#F97316] text-xs font-medium underline underline-offset-2"
            >
          ...
          {isMyPhotosLoading ? (
            <div className="flex items-center gap-2 text-white/35 text-sm py-8 justify-center">
              <Loader2 size={14} className="animate-spin" />
              Checking for your photos…
            </div>
          ) : myPhotos.length > 0 ? (
            <>
              <PhotoAlbum
                ...
                toolbar={{
                  buttons: [
                    <button
                      ...
                      onClick={() =>
                        downloadPhoto(
                          myPhotos[lightboxIndex]?.photo_url,
                          `photo_${myPhotos[lightboxIndex]?.id}.jpg`,
                        )
                      }
        ...
        <div className="max-w-2xl mx-auto">
          {isReplacingReference && (
            <button
              onClick={() => setIsReplacingReference(false)}
              className="text-white/40 hover:text-white/70 text-xs font-medium mb-4"
            >
              ← Back to your photos
            </button>
          )}
      ...
      {isCameraOpen && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex flex-col items-center justify-center p-4">
          ...
          <div className="w-full max-w-md">
            <p className="text-center text-white font-semibold text-base mb-1">Take a selfie</p>
