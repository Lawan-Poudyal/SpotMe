...
const Sidebar: React.FC<SidebarProps> = ({ activeSection, setActiveSection, onMyEventsClick }) => {
      ...
      >
                ...
                {recentEvents.map(({ label, icon }) => (
                  <button
                    ...
                    className="flex items-center gap-3 px-3 py-2 rounded-xl w-full text-left
                      text-[13.5px] text-white/45 hover:text-white/75 hover:bg-white/5
        ...
        {!collapsed && (
          <div className="px-5 py-3 border-t border-white/6">
            <p className="text-[10px] text-white/15 text-center select-none">SpotMe v1.0</p>
          </div>
        )}
      </aside>
